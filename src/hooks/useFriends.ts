import { useState, useEffect } from 'react';
import { collection, onSnapshot } from 'firebase/firestore';
import { db } from '../lib/firebase';
import { useAuth } from './useAuth';
import { sendFriendRequest, removeFriend as apiRemoveFriend } from '../services/friendService';

export function useFriends() {
  const { user, profile } = useAuth();
  const [friendUids, setFriendUids] = useState<string[]>([]);
  const [loading, setLoading] = useState<boolean>(true);

  useEffect(() => {
    if (!user) {
      setFriendUids([]);
      setLoading(false);
      return;
    }

    setLoading(true);
    const friendsRef = collection(db, 'friendships', user.uid, 'friends');
    const unsubscribe = onSnapshot(
      friendsRef,
      (snapshot) => {
        const uids = snapshot.docs
          .filter(docSnap => {
            const data = docSnap.data();
            return data.status === 'mutual' || !data.status;
          })
          .map(docSnap => docSnap.id);
        setFriendUids(uids);
        setLoading(false);
      },
      (error) => {
        console.error('Error listening to friends:', error);
        setLoading(false);
      }
    );

    return () => unsubscribe();
  }, [user]);

  const addFriend = async (friendUid: string) => {
    if (!user || !profile) throw new Error('Must be signed in to add friends.');
    const meName = profile.username || profile.displayName || user.displayName || 'User';
    const mePhoto = profile.photoURL || '';
    await sendFriendRequest(user.uid, friendUid, meName, mePhoto);
  };

  const removeFriend = async (friendUid: string) => {
    if (!user) throw new Error('Must be signed in to remove friends.');
    await apiRemoveFriend(user.uid, friendUid);
  };

  return {
    friendUids,
    loading,
    addFriend,
    removeFriend,
  };
}

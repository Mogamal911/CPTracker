...
export const EditUsernameModal: React.FC<EditUsernameModalProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    // If username is unchanged from the current profile, treat it as valid
      ...
      <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl overflow-hidden">
        ...
        <div className="space-y-6">
          <div className="space-y-2 text-center">
            <h2 className="text-2xl font-bold tracking-tight text-white">Change Your Username</h2>
            <p className="text-sm text-slate-400">
              Updating your username will release your current one for others to claim.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1">
              <label htmlFor="edit-username" className="block text-xs font-semibold uppercase tracking-wider text-slate-400">
                New Username <span className="text-rose-500">*</span>
              </label>
              <div className="relative flex items-center">
                <input
                  ...
                  className={`w-full bg-slate-950 border rounded-xl px-4 py-2.5 text-sm text-slate-200 placeholder-slate-600 focus:outline-none transition duration-150 ${
                    usernameValid && username.trim() !== (profile?.username || '')
                ...
                <div className="absolute right-3 text-xs flex items-center">
                  ...
                  {!usernameChecking && usernameValid && username.trim() !== (profile?.username || '') && <span className="text-emerald-400">✔</span>}
              ...
              {!usernameChecking && usernameValid && username.trim() !== (profile?.username || '') && (
                <p className="text-[11px] text-emerald-400">Username is available</p>
              )}
              {!usernameChecking && usernameValid && username.trim() === (profile?.username || '') && (
                <p className="text-[11px] text-slate-400 font-mono">Current username</p>
              )}
